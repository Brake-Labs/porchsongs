...
and PDF export all work with no gateway configured. Only three things need one: tidying
up a chart's formatting, rewriting lyrics, and suggesting a folder for a chart. Those
actions disable themselves and say why. Folders themselves are entirely manual and
always work; the AI suggestion is one opt-in tap on one chart.
